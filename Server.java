...
public class Server {
    ...
    static class ClientHandler extends Thread {
        ...
        User currentUser;
        // Maintain a notifications list per client.
        ...
        // A helper method to send notifications in real time and also store them.
        public void sendNotification(String message) {
            String notifMsg = "[NOTIF:] " + message;
            out.println(notifMsg);
            notifications.add(notifMsg);
        }

        // Method to view stored notifications.
        ...
        private void handleAdminRequests() throws IOException {
            while (true) {
                ...
                out.println("2. View Notifications");
        ...
        private void handleCustomerRequests() throws IOException {
            while (true) {
                // Before showing menu, check if customer has an active (non-completed) ride.
                if (hasActiveRideForCustomer()) {
                    out.println("You have an ongoing trip. Disconnect is disabled until the trip is completed.");
                }
                out.println("\nCustomer Menu:");
                out.println("1. Request Ride");
                out.println("2. View Ride Status");
                out.println("3. View Offered Fares");
                out.println("4. Accept/Decline Offer (Waiting Mode)");
                out.println("5. Rate Driver");
                out.println("6. View Notifications");
                out.println("7. Disconnect");
                out.println("Enter your choice:");
                String choice = in.readLine();
                if (choice == null) {
                    break;
                ...
                if (choice.equals("7")) {
                    if (hasActiveRideForCustomer()) {
                        out.println("You cannot disconnect while a trip is in progress.");
                        continue;
                    }
                    out.println("Disconnecting...");
                ...
                switch (choice) {
                    ...
                    case "3":
                        ...
                        break;
                    case "4":
                        // New waiting mode for accepting/declining offers.
                        waitingModeForOffers();
                        break;
                    case "5":
                        rateDriver();
                        break;
        ...
        // Check if the customer has any active ride (not Completed)
        private boolean hasActiveRideForCustomer() {
            for (Ride ride : rides) {
        ...
        // When a customer requests a ride, instead of immediately returning to the main menu,
        // we enter a waiting loop where real-time notifications are still delivered.
        private void requestRide() throws IOException {
            ...
            boolean availableDriverExists = false;
            for (Driver driver : drivers) {
                if (driver.isAvailable()) {
                    availableDriverExists = true;
                    break;
                }
            }
            if (!availableDriverExists) {
                sendNotification("There are no drivers available at the moment.");
            }
            ...
            // Enter waiting mode for this ride until an offer is accepted.
            waitingModeForOffers(ride);
        }

        // Overloaded waiting mode when a ride is already selected (used internally)
        private void waitingModeForOffers(Ride ride) throws IOException {
            out.println("You are now in waiting mode for ride ID " + ride.getId() +
                    ". You will be notified of any fare offers in real time.");
            out.println("Type 'view' to see current offers, 'accept <driver_username>' to accept an offer, or 'decline <driver_username>' to decline an offer.");
            // Loop until the ride status changes from Pending.
            while (ride.getStatus().equalsIgnoreCase("Pending")) {
                ...
                if (parts[0].equalsIgnoreCase("view")) {
                    // Show offers for this ride.
                    viewOfferedFares(ride);
                } else if (parts[0].equalsIgnoreCase("accept")) {
                    if (parts.length < 2) {
                        out.println("Usage: accept <driver_username>");
                        continue;
                    }
                    String driverUsername = parts[1];
                    // Find the corresponding offer.
                    RideOffer acceptedOffer = null;
                    for (RideOffer offer : ride.getOffers()) {
                    ...
                    if (acceptedOffer == null) {
                        out.println("No offer found from driver " + driverUsername + ".");
                    } else {
                        // Accept the offer.
                        ride.setDriver(acceptedOffer.getDriver());
                        ride.setStatus("Accepted");
                        out.println("You have accepted the offer from driver " + driverUsername +
                                " for ride ID " + ride.getId() + ".");
                        System.out.println("Customer " + currentUser.getUsername() +
                                " accepted the offer from driver " + driverUsername +
                                " for ride ID " + ride.getId());
                        ...
                        for (ClientHandler handler : clientHandlers) {
                            if (handler.currentUser != null &&
                                ...
                                handler.sendNotification("Your fare offer for ride ID " + ride.getId() + " has been accepted by the customer.");
                            }
                        }
                            ...
                            if (!offer.getDriver().getUsername().equals(driverUsername)) {
                                ...
                                for (ClientHandler handler : clientHandlers) {
                                    if (handler.currentUser != null &&
                                        ...
                                        handler.sendNotification("Ride ID " + ride.getId() + " has been assigned to another driver.");
                                    }
                                }
                            }
                        }
                ...
                } else if (parts[0].equalsIgnoreCase("decline")) {
                    if (parts.length < 2) {
                        out.println("Usage: decline <driver_username>");
                        continue;
                    }
                    String driverUsername = parts[1];
                    // Remove the offer from this driver if exists.
                    boolean found = false;
                    Iterator<RideOffer> iterator = ride.getOffers().iterator();
                    while (iterator.hasNext()) {
                        RideOffer offer = iterator.next();
                        if (offer.getDriver().getUsername().equals(driverUsername)) {
                            iterator.remove();
                            // Free the driver lock.
                            Server.driverLockedRide.remove(offer.getDriver());
                            out.println("Declined offer from driver " + driverUsername + ".");
                            found = true;
                            break;
                        }
                    }
                    ...
                    out.println("Invalid command. Please type 'view', 'accept <driver_username>', or 'decline <driver_username>'.");
                }
            }
            // Once the ride status is no longer Pending, break out and return to main menu.
            out.println("Exiting waiting mode. Returning to main menu.");
        }

        // This version of viewOfferedFares is used in waiting mode for a specific ride.
        private void viewOfferedFares(Ride ride) {
            if (ride.getOffers().isEmpty()) {
                out.println("No offers have been made for this ride yet.");
                return;
            }
            out.println("Offers for ride ID " + ride.getId() + ":");
            for (RideOffer offer : ride.getOffers()) {
                out.println("Driver: " + offer.getDriver().getUsername() + " | Fare: $" + offer.getFare());
            }
        }

        // The original viewOfferedFares method – asks for ride id.
        private void viewOfferedFares() throws IOException {
            ...
            viewOfferedFares(targetRide);
        }

        // An additional waiting mode command handler when not already in a ride waiting loop.
        private void waitingModeForOffers() throws IOException {
            // Let the user choose which active ride they want to manage offers for.
            out.println("Enter your active ride ID to manage offers:");
            String rideIdStr = in.readLine();
            int rideId;
            try {
                rideId = Integer.parseInt(rideIdStr);
            ...
            for (Ride ride : rides) {
                if (ride.getId() == rideId && ride.getCustomer().getUsername().equals(currentUser.getUsername())
                        && ride.getStatus().equalsIgnoreCase("Pending")) {
                    targetRide = ride;
                    break;
                }
            }
            if (targetRide == null) {
                out.println("Ride not found, not pending, or you are not the customer for this ride.");
                return;
            }
            waitingModeForOffers(targetRide);
        }
        ...
        private void handleDriverRequests() throws IOException {
            while (true) {
                // Before showing menu, check if driver has an active ride.
                if (hasActiveRideForDriver()) {
                    out.println("You have an ongoing trip. Disconnect is disabled until the trip is completed.");
                }
                out.println("\nDriver Menu:");
                ...
                out.println("5. View Notifications");
        ...
        // Check if the driver has an active ride (assigned and not completed)
        private boolean hasActiveRideForDriver() {
            for (Ride ride : rides) {
                if (ride.getDriver() != null &&
                        ride.getDriver().getUsername().equals(currentUser.getUsername()) &&
                        !ride.getStatus().equalsIgnoreCase("Completed")) {
                    return true;
                }
            }
        ...
        // (This routine is now available through waitingModeForOffers in real time.)
        ...
        private void updateRideStatus() throws IOException {
            ...
            for (Ride ride : rides) {
                ...
                if (ride.getId() == rideId) {
                    ...
                    notifyCustomer(ride.getCustomer().getUsername(), "Ride ID " + ride.getId() + " status changed to " + newStatus + ".");
